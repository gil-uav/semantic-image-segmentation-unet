...
class UNet(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Metrics
        is_multiclass = True if self.hparams.n_classes > 1 else False
        self.train_pres = pl.metrics.Precision(
            num_classes=self.hparams.n_classes, is_multiclass=is_multiclass
        )
        self.train_rec = pl.metrics.Recall(
            num_classes=self.hparams.n_classes, is_multiclass=is_multiclass
        )
        self.train_f1 = pl.metrics.classification.FBeta(
            num_classes=self.hparams.n_classes
        )

        self.val_pres = pl.metrics.Precision(
            num_classes=self.hparams.n_classes, is_multiclass=is_multiclass
        )
        self.val_rec = pl.metrics.Recall(
            num_classes=self.hparams.n_classes, is_multiclass=is_multiclass
        )
        self.val_f1 = pl.metrics.classification.FBeta(
            num_classes=self.hparams.n_classes
        )

        self.test_pres = pl.metrics.Precision(
            num_classes=self.hparams.n_classes, is_multiclass=is_multiclass
        )
        self.test_rec = pl.metrics.Recall(
            num_classes=self.hparams.n_classes, is_multiclass=is_multiclass
        )
        self.test_f1 = pl.metrics.classification.FBeta(
            num_classes=self.hparams.n_classes
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        return {
            "test_loss": loss,
            "test_precision": self.val_pres,
            "test_recall": self.val_rec,
            "test_f1": self.val_f1,
        }
